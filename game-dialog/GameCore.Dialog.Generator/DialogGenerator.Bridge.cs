...
public partial class DialogGenerator
{
    ...
    private static void GenerateBridge(SourceProductionContext spc, ImmutableArray<INamedTypeSymbol?> classSymbols)
    {
        ...
        spc.AddSource($"GameCore.Dialog.GeneratedBridge.g", SourceText.From(sourceText, Encoding.UTF8));
    }
    ...
    private static string GenerateBridgeText(List<BridgeObject> bridgeObjects)
    {
        ...
        for (int i = 0; i < bridgeObjects.Count; i++)
        {
            ...
            sb.AppendLine($$"""
                ...
                {
                    public {{x.ClassName}}(GameCore.Dialog.DialogRunner dialogRunner)
                    {
                        DialogRunner = dialogRunner;
                    }

                    public GameCore.Dialog.DialogRunner DialogRunner { get; }
                }
            """);
        ...
        namespace GameCore.Dialog
        {
            ...
            {
                public GeneratedBridge(DialogRunner dialogRunner)
                ...
                {
                    DialogBridge.Create = (dialogRunner) => new GeneratedBridge(dialogRunner);{{CollectFuncDefs(bridgeObjects, tempSB)}}{{CollectVarDefs(bridgeObjects, tempSB)}}
                }
    ...
    private static string CollectConstructorLogic(List<BridgeObject> bridgeObjects, StringBuilder tempSB)
    {
        for (int i = 0; i < bridgeObjects.Count; i++)
        {
            tempSB.Append($"\n            _userBridge_{i} = new(dialogRunner);");
        }
            ...
            foreach (var kvp in bridgeObject.FuncDefs)
            {
                ...
                tempSB.Append($$"""
                            async void InternalLocal{{func.Name}}({{GetParams(func)}})
                            {
                                ...
                                _userBridge_{{i}}.DialogRunner.Resume();
                            }
                """);
            }
        }
