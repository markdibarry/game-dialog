...
{
    public TextDocumentHandler(ILanguageServerFacade languageServer, Dialog dialog)
    {
    ...
    public override async Task<Unit> Handle(DidOpenTextDocumentParams notification, CancellationToken ct)
    {
        ...
        List<Error> errors = [];
        _dialog.LoadFromText(notification.TextDocument.Text);
        _dialog.ValidateScript(errors);
    ...
    public override async Task<Unit> Handle(DidChangeTextDocumentParams notification, CancellationToken ct)
    {
        ...
        try
        {
            ...
            List<Error> errors = [];
            _dialog.LoadFromText(notification.ContentChanges.First().Text);
            _dialog.ValidateScript(errors);
            PublishDiagnostics(notification.TextDocument.Uri, errors);
        }
        catch (Exception ex)
