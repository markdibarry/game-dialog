...
{
    public DialogReader(DialogRunner dialog)
    {
        _dialog = dialog;
    }
    ...
    private int ReadChoiceBlock()
    {
        ...
        void FillChoices(List<Choice> choices)
        {
            ...
            while (lineIdx < Script.Count)
            {
                ...
                if (DialogRunner.TranslationFileType != TranslationFileType.None && _state.RowPrefix.Length > 0)
                {
                    string key = $"{_state.RowPrefix}_Line{lineIdx}";
                    string text;

                    if (DialogRunner.TranslationFileType == TranslationFileType.CSV)
    ...
    {
        if (DialogRunner.TranslationFileType == TranslationFileType.CSV && _state.RowPrefix.Length > 0)
        {
        ...
        {
            if (DialogRunner.TranslationFileType != TranslationFileType.POT || _state.RowPrefix.Length == 0)
                return text;
