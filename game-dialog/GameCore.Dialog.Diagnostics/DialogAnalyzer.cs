...
namespace GameCore.Dialog.Diagnostics;
...
#pragma warning restore RS2008
    ...
    private static void AnalyzeNamedType(SymbolAnalysisContext context)
    {
        ...
        foreach (var attribute in attributes)
        {
            ...
            if (attr == "GameCore.Dialog.DialogTextLabelAttribute")
            {
            ...
            else if (attr == "GameCore.Dialog.DialogBridgeAttribute" && !IsPartial(namedType))
            {
                Location location = namedType.Locations.FirstOrDefault() ?? Location.None;
