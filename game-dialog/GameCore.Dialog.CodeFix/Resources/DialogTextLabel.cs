...
public partial class DialogTextLabel : RichTextLabel
{
    ...
    public void OnTextEventTriggered(TextEvent textEvent)
    {
        ...
        if (!DialogRunner.TryParseBuiltInEvent(textEvent, out EventType eventType, out float eventParam))
        {
            if (textEvent.Tag.Span.StartsWith("await "))
                Suspended = true;

            DialogRunner.TryEvaluateExpression(textEvent.Tag);
            return;
        }
    ...
    private void ResetAndSetText(string? newText = null)
    {
        ...
        if (newText != null)
        {
            string eventParsedText = DialogRunner?.ParseEventsFromText(newText, _textEvents) ?? newText;
            Text = eventParsedText;
            string displayedText = GetParsedText();
            CachedText = displayedText;
            DialogRunner.AdjustEventIndices(eventParsedText, displayedText, _textEvents);
        }
        ...
        PauseTimer = 0;
        AutoProceedEnabled = DialogRunner?.GlobalAutoProceedEnabled ?? false;
        AutoProceedTimeout = DialogRunner?.GlobalAutoProceedTimeout ?? 0;
        SpeedMultiplier = DialogRunner?.GlobalSpeedMultiplier ?? 1;
    }
    ...
    {
        if (DialogRunner == null || _textEventIndex >= _textEvents.Count)
            return false;
