...
public partial class MainDialogVisitor
{
    ...
    private List<int>? GetAssignTagStmt(AssignmentContext context)
    {
        ...
        if (expName is BuiltIn.SPEED or BuiltIn.PAUSE or BuiltIn.AUTO)
        {
            ...
            if (expName is BuiltIn.AUTO or BuiltIn.SPEED && value < 0)
            {
                _diagnostics.AddError(context, "Invalid value: Cannot be less than zero.");
                return null;
            }
            else if (expName is BuiltIn.PAUSE && value <= 0)
            {
