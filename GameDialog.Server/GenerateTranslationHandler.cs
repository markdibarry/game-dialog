...
{
    public GenerateTranslationHandler(ILanguageServerFacade server, ILanguageServerConfiguration configuration, Dialog dialog)
    {
        _server = server;
    ...
    private List<string> CreateTranslation(bool isCSV)
    {
        ...
        if (isCSV)
        {
            Dialog.TranslationFileType = TranslationFileType.CSV;
            sw.Write("keys,en");
        }
        else
        {
            Dialog.TranslationFileType = TranslationFileType.POT;
            sw.WriteLine("msgid \"\"");
            sw.WriteLine("msgstr \"\"");
        }
        ...
        {
            _dialog.LoadFromFile(filePath, rootPath);
            _dialog.ValidateScript(errors, null, sw);
